...
| 장점 | 설명 |
|------|------|
| **데이터 무결성** | 트랜잭션 성공 여부만 존재, 중간 상태 없음 |
| **일관성 보장** | 제약 조건, 외래 키로 데이터 정합성 유지 |
| **동시성 제어** | Isolation으로 동시 접근 시에도 데이터 안전 |
| **장애 복구** | Durability로 커밋된 데이터는 영구 보존 |
| **예측 가능성** | ACID 보장으로 개발자가 안심하고 코딩 |

## 단점

| 단점 | 설명 |
|------|------|
| **성능 오버헤드** | 트랜잭션 관리, Lock, 로그 쓰기 비용 발생 |
| **Lock 경합** | 동시성 높을수록 Lock 대기 시간 증가 |
| **확장성 제한** | Isolation 강도가 높을수록 동시 처리 감소 |
| **복잡성** | 트랜잭션 범위 설계, 데드락 처리 필요 |
