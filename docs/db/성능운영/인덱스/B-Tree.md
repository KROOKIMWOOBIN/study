...
key와 실제 데이터 위치가 내부 노드에도 들어갈 수 있다.
검색 대상 key가 내부 노드에 있으면
아래로 더 내려가지 않아도 된다.
```
...
이 PK 값으로 PRIMARY KEY B+Tree를 다시 탐색한다.
```
