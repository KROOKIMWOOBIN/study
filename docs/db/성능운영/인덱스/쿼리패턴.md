...
인덱스는 내부적으로 정렬된 자료구조지만, SQL 조회 결과의 행 순서는 `ORDER BY`가 있을 때만 보장된다. 옵티마이저가 어떤 인덱스를 탈지, 어떤 실행 계획을 선택할지는 상황에 따라 달라질 수 있으므로 `ORDER BY` 없는 결과 순서에 의존하면 안 된다.
...
정렬이 필요한 쿼리는 반드시 `ORDER BY`를 명시한다. 인덱스는 그 `ORDER BY`를 더 빠르게 처리하도록 도와주는 역할이다.
