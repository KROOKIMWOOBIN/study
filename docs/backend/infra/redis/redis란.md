...
| Persistence | RDB / AOF 방식으로 Redis 데이터를 디스크에 남김 | 재시작 복구용 저장 |
...
Redis는 RDBMS 대체재라기보다 RDBMS 앞에서 **반복 조회와 짧은 상태 처리 비용을 줄이는 역할**로 쓰는 경우가 많습니다.
...
| 복잡한 조인과 검색 | 낮음 | RDBMS 역할이나 검색 엔진 역할이 아님 |
